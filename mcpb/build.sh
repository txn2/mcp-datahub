...
echo "Building MCPB bundles for mcp-datahub v${VERSION}"
...
for platform in "${PLATFORMS[@]}"; do
    ...
    BUNDLE_DIR="$BUILD_DIR/mcp-datahub-${PLATFORM_NAME}"
    ...
    if [ "$GOOS" = "windows" ]; then
        BINARY_NAME="mcp-datahub.exe"
    fi

    if [ "$USE_DIST" = true ]; then
        # Use goreleaser's pre-built binary from archive
        # GoReleaser creates archives like mcp-datahub_1.0.0_darwin_amd64.tar.gz
        ARCHIVE_PATTERN="$PROJECT_ROOT/dist/mcp-datahub_${VERSION}_${DIST_SUFFIX}"
        ...
        if [ "$GOOS" = "windows" ]; then
            ...
            if [ -f "$ARCHIVE" ]; then
                ...
                (cd "$BUNDLE_DIR" && tar -xzf "$ARCHIVE" && mv */mcp-datahub . 2>/dev/null || true)
            fi
        fi
        ...
        CGO_ENABLED=0 GOOS="$GOOS" GOARCH="$GOARCH" go build \
            -trimpath \
            -ldflags="-s -w -X github.com/txn2/mcp-datahub/internal/server.Version=${VERSION}" \
            -o "$BUNDLE_DIR/$BINARY_NAME" \
            "$PROJECT_ROOT/cmd/mcp-datahub"
    fi
    ...
    ICON_FILE="$PROJECT_ROOT/docs/images/logo.png"
    if [ -f "$ICON_FILE" ]; then
    ...
    MCPB_FILE="$BUILD_DIR/mcp-datahub_${VERSION}_${GOOS}_${GOARCH}.mcpb"
