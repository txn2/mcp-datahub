...
// CreateDocumentInput holds the parameters for creating a new document.
type CreateDocumentInput struct {
	// Title is the document title (required).
	Title string `json:"title"`

	// Content is the document body text (required).
	Content string `json:"content"`

	// SubType is the optional document sub-type.
	SubType string `json:"sub_type,omitempty"`

	// State is the initial state: PUBLISHED (default) or UNPUBLISHED.
	State string `json:"state,omitempty"`

	// ShowInGlobalContext controls visibility (default: true).
	ShowInGlobalContext *bool `json:"show_in_global_context,omitempty"`

	// RelatedAssets are entity URNs to link to the document.
	RelatedAssets []string `json:"related_assets,omitempty"`
}

// UpdateDocumentInput holds the parameters for updating a document.
type UpdateDocumentInput struct {
	// Title is the new title (optional; omit to keep current).
	Title string `json:"title,omitempty"`

	// Content is the new body text (optional; omit to keep current).
	Content string `json:"content,omitempty"`

	// State is the new state (optional; omit to keep current).
	State string `json:"state,omitempty"`

	// ShowInGlobalContext updates visibility (optional; nil to keep current).
	ShowInGlobalContext *bool `json:"show_in_global_context,omitempty"`

	// RelatedAssets replaces linked entity URNs (optional; nil to keep current).
	RelatedAssets []string `json:"related_assets,omitempty"`
}

// DocumentSearchResult holds results from a document search.
type DocumentSearchResult struct {
	// Total is the total number of matching documents.
	Total int `json:"total"`

	// Documents are the matching documents.
